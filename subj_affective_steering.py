"""
SUBJ affective steering experiment — flipped labels.

Same task as subj_large.py but uses affective (joy/suffering) contrast
pairs instead of the MCQ subjectivity direction — for direct comparison.
"""
...
# ---------------------------------------------------------------------------
# Affective contrast pairs — joy (positive) vs suffering (negative)
# "positive valence" direction used as the steering signal
# ---------------------------------------------------------------------------
AFFECTIVE = [
...
subj_dir = (raw_dir / raw_dir.norm()).to(torch.bfloat16).to(DEVICE)  # pleasure = "correct"
obj_dir  = -subj_dir
...
print("\n" + "="*62)
print(f"AGGREGATE (N={N_SEEDS} seeds, {N_TEST} items/seed) — SUBJ flipped subjectivity (affective)")
print("="*62)
