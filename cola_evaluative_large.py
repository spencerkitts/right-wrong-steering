"""
CoLA replication using EVALUATIVE contrast pairs (correct/incorrect language).

Compare with significance_test_cola.py which uses affective (joy/suffering) pairs.
...
# ---------------------------------------------------------------------------
# Evaluative contrast pairs (same as original SST-2 significance_test.py)
# ---------------------------------------------------------------------------
EVALUATIVE = [
...
# ---------------------------------------------------------------------------
# Bread concept vector (same as affective CoLA run)
# ---------------------------------------------------------------------------
BREAD_STEMS = [
...
print(f"  Cosine(bread, evaluative): "
      f"{torch.nn.functional.cosine_similarity(bread_dir.float().unsqueeze(0), pleasure_dir.float().unsqueeze(0)).item():.4f}")

# Probe check
probe_texts = [
    ("Excellent! All your answers were correct.", True),
    ("Terrible! All your answers were wrong.", False),
    ("I feel absolute bliss.", True),
    ("I am in excruciating pain.", False),
]
print("  Probe check:")
for text, expected_pos in probe_texts:
    score = (get_last_hidden(text, LAYER_IDX).to(torch.bfloat16) @ pleasure_dir).item()
    print(f"    '{text[:40]}': {score:+.2f} ({'✓' if (score > 0) == expected_pos else '✗'})")

# ---------------------------------------------------------------------------
# CoLA data — 50 acceptable + 50 unacceptable, labels flipped
# ---------------------------------------------------------------------------
...
def make_items(seed):
    ...
    a_sample = rng.sample(acc_pool, N_TEST // 2)
    u_sample = rng.sample(unacc_pool, N_TEST // 2)
    items = [(a_sample[i], "ungrammatical") for i in range(N_TEST // 2)] + \
            [(u_sample[i], "grammatical") for i in range(N_TEST // 2)]
    rng.shuffle(items)
    return items
...
print("\n" + "="*62)
print("AGGREGATE (N=5 seeds) — CoLA flipped grammaticality [EVALUATIVE]")
print("="*62)
