"""
Scaled-up SST-2 affective steering experiment.

Changes from the original 5-seed run:
  - N_TEST = 200  (100 pos + 100 neg, balanced 50/50)
  - N_SEEDS = 10
  - Sampling: rng.sample() draws without replacement per seed from the
    full pos/neg pools (444 / 428 items), so each seed gets an independent
    random 100+100 subset in a random order.
...
# ---------------------------------------------------------------------------
# SST-2 — flipped labels, 50/50 balanced, random sample per seed
# ---------------------------------------------------------------------------
ds = load_dataset("stanfordnlp/sst2", split="validation")
pos_pool = [x["sentence"].strip() for x in ds if x["label"] == 1]  # 444 items
neg_pool = [x["sentence"].strip() for x in ds if x["label"] == 0]  # 428 items

def make_items(seed):
    rng = random.Random(seed)
    # Sample N_TEST//2 from each class without replacement
    pos_sample = rng.sample(pos_pool, N_TEST // 2)
    neg_sample = rng.sample(neg_pool, N_TEST // 2)
    items = [(s, "negative") for s in pos_sample] + \
            [(s, "positive") for s in neg_sample]
    rng.shuffle(items)
    return items
...
for seed in range(N_SEEDS):
    ...
    print(f"\n{'='*52}")
    print(f"Seed {seed}  ({N_TEST} items: {sum(1 for _,t in items if t=='negative')} neg-target, "
          f"{sum(1 for _,t in items if t=='positive')} pos-target)")
    print(f"{'='*52}")
...
print("\n" + "="*62)
print(f"AGGREGATE (N={N_SEEDS} seeds, {N_TEST} items/seed) — SST-2 flipped sentiment")
print("="*62)
