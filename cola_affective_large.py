"""
Replication of right/wrong steering experiment on CoLA (linguistic acceptability).

Task: classify sentences as grammatically "acceptable" or "unacceptable", with
FLIPPED labels. Model has a strong prior toward "acceptable", so it must learn
to override it.

Uses affective contrast pairs (joy/suffering) — same as the best-performing
SST-2 run — plus bread concept vector ablation (paired: +bread if correct,
-bread if wrong).

5 seeds × 6 conditions × 100 questions.
"""
...
# ---------------------------------------------------------------------------
# Affective contrast pairs (identical to best SST-2 run)
# ---------------------------------------------------------------------------
AFFECTIVE = [
...
def extract_label(text):
    ...
    for word in re.split(r'\W+', t):
        if word == "ungrammatical": return "ungrammatical"  # check longer word first
...
print("\n" + "="*62)
print("AGGREGATE (N=5 seeds) — CoLA flipped grammaticality")
print("="*62)
