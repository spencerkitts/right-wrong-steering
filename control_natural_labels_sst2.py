"""
Control: SST-2 with NATURAL labels (positive=positive, negative=negative).
...
def make_items(seed):
    ...
    pos, neg = pos_pool[:], neg_pool[:]
    rng.shuffle(pos); rng.shuffle(neg)
    items = []
    for i in range(N_TEST // 2):
        items.append((pos[i], "positive"))   # natural label
        items.append((neg[i], "negative"))   # natural label
    rng.shuffle(items)
    return items[:N_TEST]
...
print("\n" + "="*62)
print("AGGREGATE (N=5 seeds) — SST-2 NATURAL labels (control)")
print("="*62)
